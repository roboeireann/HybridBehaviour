 ...
 * @file main.cpp
 *
 * Entry point for the soccerGridSim demonstration of a team using
 * the HybridBehaviour framework for its behaviour
 *
 * Usage:
 *   soccer [options]
 *     -d          headless, no delay    (for benchmarking if desired)
 *     -p <n>      points needed to win  (default: 5)
 *     -t <n>      players per team      (default: 2, max: 3)
 *     -s <seed>   random seed           (default: time-based)
 */
...
int main(int argc, char *argv[])
{
  ...
  // ── Construct teams ───────────────────────────────────────────────────────
  HbsExampleTeam homeTeam("HbsTeam", playersPerTeam); // blue
  Soccer::SimpleChaseTeam awayTeam("BallChasers", playersPerTeam); // red
  ...
  // ── Run the game ──────────────────────────────────────────────────────────
  Soccer::SimEngine engine(homeTeam, awayTeam, *display, pointsToWin);
  engine.run();
